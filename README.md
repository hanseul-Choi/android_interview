...
액티비티는 독립적으로 활용이 가능하며, 프래그먼트는 액티비티에 종속되어있다. <br>
또한, 액티비티는 전체화면을 구성하지만, 프래그먼트는 부분적으로 화면을 제공할 수 있다. <br>
마지막으로 액티비티는 스택에서 관리하고 프래그먼트는 트랜잭션을 통해 관리한다.
...
원격 서비스의 경우, 같은 기기에 실행되고 있는 다른 애플리케이션에서 서비스를 실행할 수 있다. (Fragment Manager에서 관리)
